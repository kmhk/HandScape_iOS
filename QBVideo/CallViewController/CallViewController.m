...
- (void)viewDidLoad {
    ...
    _imgPhoto.layer.cornerRadius = _imgPhoto.frame.size.width / 2;
    _imgPhoto.clipsToBounds = YES;
    _imgPhoto.contentMode = UIViewContentModeScaleAspectFill;
    _imgPhoto.layer.borderWidth = 1.0;
    _imgPhoto.layer.borderColor = [UIColor colorWithRed:0 green:97/255.0 blue:147.0/255.0 alpha:1.0].CGColor;
	...
	self.view.backgroundColor = self.opponentsCollectionView.backgroundColor = [UIColor whiteColor];//[UIColor colorWithRed:0.1465 green:0.1465 blue:0.1465 alpha:1.0];
    ...
    QBUUser *targetUser = [users objectAtIndex:1];
    UIImage *targetPhoto = [_gDKeeper.photoDict objectForKey:STR_L(targetUser.blobID)];
    
    if(targetPhoto != nil)
        [_imgPhoto setImage:targetPhoto];
    else
        [_imgPhoto setImage:[UIImage imageNamed:@"placeholder_regular.png"]];

    _lblUserName.text = targetUser.fullName;
}

- (void) setLabelState:(QBRTCConnectionState) connectionState{
...
- (void)configureGUI {
    ...
    if (self.session.conferenceType == QBRTCConferenceTypeVideo) {
        ...
        self.videoEnabled = FALSE;//[QBButtonsFactory videoEnable];
...
//    NSString *markerText = [NSString stringWithFormat:@"%lu", (unsigned long)user.index + 1];
    NSString *markerText = [NSString stringWithFormat:@"%@", user.fullName];
    [cell setColorMarkerText:markerText andColor:[UIColor clearColor]];
    [cell setUserPhotoImg:[_gDKeeper.photoDict objectForKey:STR_L(user.blobID)]];
    return cell;
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
	...
	return CGSizeMake(collectionView.frame.size.width, collectionView.frame.size.height / self.users.count);
}
...
//            if(userID != [NSNumber numberWithInteger:_gDKeeper.currentQBUser.ID])
            {
                [self setLabelState:[self.session connectionStateForUser:userID]];
            }
        }];
    }
}
